-- Alpha Hunter geometry diagnostics v0.2.1 corrective migration
-- Fixes ambiguous source aliases discovered by the production smoke test.
-- Keeps the v0.2 safety contract unchanged: diagnostics only, no thresholds,
-- no T0/READY authority, no risk/leverage change, no exchange/order call.
...
begin
  ...
  if v_run_id is null then
    return jsonb_build_object(
      ...
      'model_version','geometry-diagnostics-v0.2.1-volatility-context',
      'shadow_only',true,
  ...
  with bridge as (
    ...
    select
      ...
      sf.source_payload#>>'{timeframes,1H,indicators,volume_anomaly,state}' as source_volume_state_1h,
      sf.source_payload->>'market_phase' as source_market_phase,
      sf.source_payload->>'opportunity_timing' as source_opportunity_timing
    from bridge b
    left join lateral (
    ...
    select
      md5('geometry-diagnostics-v0.2.1-volatility-context|'||m.run_id||'|'||m.symbol||'|'||m.direction),
      ...
      'geometry-diagnostics-v0.2.1-volatility-context',true,false
    from metrics m
  ...
  from (
    ...
    where run_id=v_run_id and model_version='geometry-diagnostics-v0.2.1-volatility-context'
...
-- Wrapper is recreated unchanged so the fixed function is what the scheduled call executes.
...
-- Re-enable the diagnostic wrapper only after this corrected migration is applied.
do $$
declare
  v_job_id bigint;
begin
