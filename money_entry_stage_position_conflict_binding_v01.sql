-- Alpha Hunter P0 Money Entry position-conflict binding v0.1
-- Forward-only. No historical stage mutation/backfill. No threshold activation/relaxation.
-- Requires canonical-account-ledger-v0.1 evidence. Missing or non-verified account
-- evidence remains NULL and preserves the existing fail-closed blocker.

create or replace function private.alpha_hunter_stage_position_conflict(
  p_symbol text,
...
returns table(
  ...
  account_captured_at_utc timestamptz,
...
as $$
  select
    ...
    (select count(*)::integer
      ...
      where p.account_snapshot_id=a.account_snapshot_id) as open_position_count,
    exists(
      select 1
      from public.alpha_hunter_open_position_snapshots p
      where p.account_snapshot_id=a.account_snapshot_id
        and p.symbol=upper(p_symbol)
    ) as position_conflict
  from public.alpha_hunter_account_state_snapshots a
  where a.source='CANONICAL_SCANNER_PRIVATE_ACCOUNT_CACHE'
    and a.connection_status='CONNECTED_READ_ONLY'
    and a.schema_validated=true
    and a.complete=true
    and a.shadow_only=true
    and a.trade_permission=false
    and a.evidence->>'canonical_run_id'=p_source_run_id
    and a.evidence->>'cached_payload_only'='true'
    and a.evidence->>'no_extra_bitget_request'='true'
    and coalesce(a.evidence->>'persisted_open_position_count','') ~ '^[0-9]+$'
    and (a.evidence->>'persisted_open_position_count')::integer = (
      select count(*)::integer
      from public.alpha_hunter_open_position_snapshots p
      where p.account_snapshot_id=a.account_snapshot_id
    )
    and a.captured_at_utc<=p_source_captured_at_utc
    and a.captured_at_utc>=date_trunc('hour',p_source_captured_at_utc)
  order by a.captured_at_utc desc
  limit 1;
$$;

revoke all on function private.alpha_hunter_stage_position_conflict(text,text,timestamptz) from public,anon,authenticated;
grant execute on function private.alpha_hunter_stage_position_conflict(text,text,timestamptz) to service_role;
...
begin
  ...
  with src as (
    select
      ...
      pc.account_snapshot_id position_account_snapshot_id,
      pc.account_captured_at_utc position_account_captured_at_utc,
      pc.open_position_count verified_open_position_count,
      pc.position_conflict canonical_position_conflict
    from public.alpha_hunter_big_mover_money_entry_shadow b
    left join lateral (
    ...
    left join lateral private.alpha_hunter_stage_position_conflict(
      b.symbol,b.run_id,b.captured_at_utc
    ) pc on true
    ...
    select s.*,
      ...
      s.canonical_position_conflict open_position_conflict,
    ...
    select
      md5('money-entry-stage-single-writer-v0.3-position-conflict-bound|'||s.bridge_id),
      ...
      jsonb_build_object(
        ...
        'position_conflict_binding_source','CANONICAL_ACCOUNT_LEDGER_SAME_RUN_SAME_HOUR_NON_FUTURE',
        'position_account_snapshot_id',s.position_account_snapshot_id,
        'position_account_captured_at_utc',s.position_account_captured_at_utc,
        'verified_open_position_count',s.verified_open_position_count,
        'position_conflict_verified',s.open_position_conflict is not null,
        'stable_episode_id_status','NOT_BOUND_TO_STABLE_EPISODE',
        'stage_snapshot_is_contemporaneous',true
      ),
      'money-entry-stage-single-writer-v0.3-position-conflict-bound',true,false
    from staged s
  ...
  return jsonb_build_object(
    ...
    'position_conflict_binding','CONNECTED_CANONICAL_ACCOUNT_SAME_RUN_SAME_HOUR_NON_FUTURE',
