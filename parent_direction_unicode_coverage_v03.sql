-- Alpha Hunter parent-direction Unicode-safe full-coverage collector v0.3
...
begin
  ...
  loop
    foreach v_tf in array array['12H','1D'] loop
      ...
      v_endpoint := 'https://api.bitget.com/api/v3/market/candles?category=USDT-FUTURES&symbol='||extensions.urlencode(v_symbol.symbol::varchar)||'&interval='||v_tf||'&limit=60';
      ...
      loop
        insert into public.alpha_hunter_big_mover_parent_direction_shadow(
          ...
          'big-mover-parent-direction-shadow-v0.3-unicode-safe-full-money-entry-coverage',true,false,now()
        )
