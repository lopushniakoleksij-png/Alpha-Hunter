...
.two{display:grid;grid-template-columns:1fr 1fr;gap:18px}.note{color:var(--muted);font-size:12px;margin-top:8px}
@media(max-width:900px){.grid{grid-template-columns:repeat(2,1fr)}.two{grid-template-columns:1fr}.wrap{padding:14px}}
</style></head><body><div class="wrap">
<div class="top"><div><h1>Performance Analytics</h1><div class="sub">Directional accuracy and model calibration</div></div><a href="/">← Scanner</a></div>
...
<div class="card"><div class="label">Directional Accuracy</div><div class="value">{{data.overall.directional_accuracy}}%</div><div class="note">Excludes ±{{data.methodology.noise_threshold_pct}}% noise</div></div>
...
<div class="card"><div class="label">Big-Move Hit Rate</div><div class="value">{{data.overall.big_move_rate}}%</div><div class="note">≥ {{data.methodology.big_move_threshold_pct}}%</div></div>
</div>

<div class="two">
<div class="panel"><h2>Strategy Ranking</h2><table><thead><tr><th>State</th><th>Samples</th><th>Accuracy</th><th>Coverage</th><th>Avg Return</th></tr></thead><tbody>
{% for r in data.strategies %}<tr><td><b>{{r.name}}</b></td><td>{{r.samples}}</td><td>{{r.directional_accuracy}}%</td><td>{{r.coverage_pct}}%</td><td class="{{'good' if r.average_return_pct>=0 else 'bad'}}">{{r.average_return_pct}}%</td></tr>{% endfor %}
</tbody></table></div>
...
</tbody></table><div class="note">No weight changes before {{data.methodology.minimum_recommendation_samples}} samples per strategy.</div></div>
</div>

<div class="two">
<div class="panel"><h2>Score Calibration</h2><table><thead><tr><th>Score</th><th>Samples</th><th>Accuracy</th><th>Avg Return</th></tr></thead><tbody>
{% for r in data.score_buckets %}<tr><td>{{r.name}}</td><td>{{r.samples}}</td><td>{{r.directional_accuracy}}%</td><td class="{{'good' if r.average_return_pct>=0 else 'bad'}}">{{r.average_return_pct}}%</td></tr>{% endfor %}
</tbody></table></div>

<div class="panel"><h2>Confidence Calibration</h2><table><thead><tr><th>Confidence</th><th>Samples</th><th>Actual Accuracy</th><th>Avg Return</th></tr></thead><tbody>
{% for r in data.confidence_buckets %}<tr><td>{{r.name}}</td><td>{{r.samples}}</td><td>{{r.directional_accuracy}}%</td><td class="{{'good' if r.average_return_pct>=0 else 'bad'}}">{{r.average_return_pct}}%</td></tr>{% endfor %}
</tbody></table></div>
</div>
</div></body></html>
"""
