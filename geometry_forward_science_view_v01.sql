-- Alpha Hunter recovered-geometry forward science view v0.1
-- Read-only prospective exploratory evaluation using the EXISTING Money Scorecard outcomes.
-- No new market calls, no outcome writer, no T0/READY authority, no threshold derivation.

create or replace view public.alpha_hunter_geometry_forward_observations_v01
...
with geometry as (
  ...
  where g.model_version='geometry-diagnostics-v0.2.1-volatility-context'
...
select
  ...
  case when (j.evidence->>'observed_spread_pct') ~ '^-?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
    then (j.evidence->>'observed_spread_pct')::double precision end as observed_spread_pct,
  case when (j.evidence->>'observed_atr_15m_pct') ~ '^-?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
    then (j.evidence->>'observed_atr_15m_pct')::double precision end as observed_atr_15m_pct,
  case when (j.evidence->>'observed_atr_1h_pct') ~ '^-?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
    then (j.evidence->>'observed_atr_1h_pct')::double precision end as observed_atr_1h_pct,
  case when (j.evidence->>'stop_to_spread_multiple') ~ '^-?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
    then (j.evidence->>'stop_to_spread_multiple')::double precision end as stop_to_spread_multiple,
  case when (j.evidence->>'stop_to_atr_15m_multiple') ~ '^-?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
    then (j.evidence->>'stop_to_atr_15m_multiple')::double precision end as stop_to_atr_15m_multiple,
  case when (j.evidence->>'stop_to_atr_1h_multiple') ~ '^-?([0-9]+([.][0-9]*)?|[.][0-9]+)([eE][+-]?[0-9]+)?$'
    then (j.evidence->>'stop_to_atr_1h_multiple')::double precision end as stop_to_atr_1h_multiple,
  j.scorecard_id,
...
grant select on public.alpha_hunter_geometry_forward_observations_v01 to service_role;

create or replace view public.alpha_hunter_geometry_forward_status_v01
with (security_invoker=true)
as
with base as (
  select * from public.alpha_hunter_geometry_forward_observations_v01
), keys as (
  select
...
revoke all on public.alpha_hunter_geometry_forward_status_v01 from public,anon,authenticated;
