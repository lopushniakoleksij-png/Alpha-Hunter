...
-- Bind contemporaneous public Bitget maker/taker fee evidence into the existing
-- execution-cost evidence writer without activating or inventing a cost model.
-- Slippage, round-trip modeled cost, estimated cost-R and realistic net R remain
-- withheld until a separately validated full cost model exists.
do $patch$
declare
  v_def text;
...
begin
  ...
  if position(v_old_base in v_def)=0 then
    raise exception 'execution cost base binding contract drifted; refusing patch';
  end if;
  if position(v_old_fee_values in v_def)=0 then
    raise exception 'execution cost fee value contract drifted; refusing patch';
  end if;
  ...
  v_def := replace(v_def,v_old_evidence,v_new_evidence);
  execute v_def;
end;
$patch$;
