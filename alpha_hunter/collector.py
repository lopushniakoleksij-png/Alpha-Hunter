...
def validate_canonical_market_freshness(
    ...
    *,
    ...
    """Fail closed when canonical Bitget ticker evidence is stale or un-timestamped."""
    if not tickers:
    ...
    timestamps = [
        ...
        for timestamp in [_exchange_ts_ms(row)]
        if timestamp is not None
    ]
    if not timestamps:
        raise BitgetAPIError(
            "Canonical Bitget ticker payload has no exchange timestamps; "
    ...
    if age_ms < -60_000:
        raise BitgetAPIError(
            "Canonical Bitget ticker timestamp is "
            f"{abs(age_ms) / 1000:.1f}s in the future"
        )
    if age_ms > max_age_seconds * 1000:
        raise BitgetAPIError(
            "Canonical Bitget ticker payload stale: "
            f"age={age_ms / 1000:.1f}s limit={max_age_seconds}s"
        )
...
def main() -> int:
    ...
    max_market_age_seconds = int(
        config.get(
            "canonical_market_max_age_seconds",
            120,
        )
    )
    try:
